...
class Library {
    ...
    int lendBook(int uid, int bid) {
        ...
        if (lentIterator != lentBooks.end()) {
            //nie mozna wyporzyczyc wyporzyczoneh ksiazki
            return -3;
        }
    ...
    std::map < int, int > searchBoth(std::string btitle, std::string name, std::string lastname) {
        ...
        for (lentIterator = lentBooks.begin(); lentIterator != lentBooks.end(); ++lentIterator) {
            bookIterator = allBooks.find(lentIterator -> first);
            personIterator = allPpl.find(lentIterator -> second);
            tmpS1 = bookIterator -> second.btitle.substr(0, btitle.size());
            tmpS2 = personIterator -> second.name.substr(0, name.size());
            tmpS3 = personIterator -> second.last_name.substr(0, lastname.size());
            std::for_each(tmpS1.begin(), tmpS1.end(), [](char & c) {
                c = ::toupper(c);
            });
            std::for_each(tmpS2.begin(), tmpS2.end(), [](char & c) {
            ...
            std::for_each(tmpS3.begin(), tmpS3.end(), [](char & c) {
                c = ::toupper(c);
            });
            if (tmpS1 == btitle && tmpS2 == name && tmpS3 == lastname) {
                tmpM.insert(std::pair < int, int > (lentIterator -> first, lentIterator -> second));
            }
    ...
    int editBook(int bid, std::string n_btitle, std::string n_bauthor) {
        ...
        lentIterator = lentBooks.find(bid);
        if (lentIterator != lentBooks.end()) {
            //nie wolo edytowac wyporzyczonej ksziazki
            return -2;
        }
